...
# depricated 
def isbn_conflict(collection, testISBN) :
...
def add_book(collection, title, author, genre, isbn, tags) :
    if book_with_isbn(collection, isbn): # optional extension 
...
def book_with_isbn(collection, isbn):
    found = ()
...
def search_by_isbn(collection, isbn) :
    ...
    found =  book_with_isbn(collection, isbn)
    if found :
        print("We have: ")
...
try :
    assert add_book(testCollection, testBook["title"], testBook["author"], testBook["genre"], testBook["isbn"], testBook["tags"]) == [testBook], "adding book to collection function failed"
    assert tuple_to_set((1,2)) == {1,2}, "tuple to set function failed"
    assert book_with_isbn(testCollection, 1234) == (testBook["title"], testBook["author"]), "search by isbn function failed"
    assert books_with_tags(testCollection, {"t1"}) == [testBook], "search by tags function failed"
...
library = add_book(library, "Aye, and Gomorrah", "Samuel R. Delany", SCIFI, 9780375706714, 
...
library = add_book(library, "Myth and Guilt: The Crime and Punishment of Mankind", "Theodor Reik", CRIT, 9781199370785, 
                  {"film studies",                                                                                                             
                    "moral psychology", 
...
library = add_book(library, "Cyberfeminism Index", "Mindy Seu", CRIT, 9781941753514,
                  {"cyberfeminism",
...
library = add_book(library, "Dyani White Hawk: Love Language", "Tarah Hogue and Siri Engberg", ART, 9781935963349,
...
library = add_book(library, "A Canticle for Leibowitz", "Walter M. Miller Jr.", SCIFI, 9780060892999,
...
library = add_book(library, "God Bless You, Mr. Rosewater", "Kurt Vonnegut", FICT, 9780385333474,
                  {"satire",
                   "wealth inequality",
                   "philanthropy",
                   "American capitalism",
...
library = add_book(library, "Glitch Feminism: A Manifesto", "Legacy Russell", CRIT, 9781786632661,
                  {"cyberfeminism",
...
library = add_book(library, "This Can't Be the Place: Alternative Theories of the Internet", "August Kaasa Sundgaard and Ruben Stoffelen", CRIT, 9789083672120,
                  {"internet culture",
                   "network theory",
                   "digital politics",
                   "platform critique",
...
search_by_tags(library, tagsToSearch1)
search_by_tags(library, tagsToSearch2)
search_by_tags(library, tagsToSearch3)
search_by_tags(library, tagsToSearch4)

# search by isbn
isbn1 = 9781935963349
isbn2 = 9781945963349 # does not exist 

search_by_isbn(library, isbn1)
search_by_isbn(library, isbn2)